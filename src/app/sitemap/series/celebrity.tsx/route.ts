...
const SERIE_NAME = "Celebrity"; // Cambiar para cada serie
...
async function getImages(): Promise<string[]> {
  ...
  const works: Work[] = seriesData.Celebrity || [];
  return works.map(w => {
    // convert e.g. /serigraphie/Celebrity/pierre-arnould-artiste-tondo-1.jpg
    // into /serigraphie/Celebrity-mini/pierre-arnould-artiste-tondo-1.webp
    const webp = w.image.replace("/plexiglas/Celebrity/", "/plexiglas/Celebrity-mini/").replace(/\.[a-zA-Z]+$/i, ".webp");
