...
const SERIE_NAME = "anamorphose"; // Cambiar para cada serie
...
async function getImages(): Promise<string[]> {
  ...
  const works: Work[] = seriesData.anamorphose || [];
  return works.map(w => {
    // convert e.g. /serigraphie/anamorphose/pierre-arnould-artiste-tondo-1.jpg
    // into /serigraphie/anamorphose-mini/pierre-arnould-artiste-tondo-1.webp
    const webp = w.image.replace("/anamorphose/", "/anamorphose-mini/").replace(/\.[a-zA-Z]+$/i, ".webp");
