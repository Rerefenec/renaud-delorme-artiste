...
const SERIE_NAME = "cartes"; // Cambiar para cada serie
...
async function getImages(): Promise<string[]> {
  ...
  const works: Work[] = seriesData.cartes || [];
  return works.map(w => {
    ...
    const webp = w.image.replace("/cartes-mere/", "/cartes-mere-mini/").replace(/\.[a-zA-Z]+$/i, ".webp");
