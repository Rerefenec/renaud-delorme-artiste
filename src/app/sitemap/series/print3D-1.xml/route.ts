...
const SERIE_NAME = "print3D"; // Cambiar para cada serie
...
async function getImages(): Promise<string[]> {
  ...
  const works: Work[] = seriesData.print3D || [];
  return works.map(w => {
    ...
    const webp = w.image.replace("/print3D/", "/print3D-mini/").replace(/\.[a-zA-Z]+$/i, ".webp");
