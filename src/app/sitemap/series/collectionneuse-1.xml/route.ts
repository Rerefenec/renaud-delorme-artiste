...
const SERIE_NAME = "collectionneuse"; // Cambiar para cada serie
...
async function getImages(): Promise<string[]> {
  ...
  return works.map(w => {
    ...
    const webp = w.image.replace("/serigraphie/la-collectionneuse-d-art/", "/serigraphie/la-collectionneuse-d-art-mini/").replace(/\.[a-zA-Z]+$/i, ".webp");
