...
const SERIE_NAME = "hommes"; // Cambiar para cada serie
...
async function getImages(): Promise<string[]> {
  ...
  const works: Work[] = seriesData.hommes || [];
  return works.map(w => {
    // convert e.g. /serigraphie/hommes/pierre-arnould-artiste-tondo-1.jpg
    // into /serigraphie/hommes-mini/pierre-arnould-artiste-tondo-1.webp
    const webp = w.image.replace("/plexiglas/hommes/", "/plexiglas/hommes-mini/").replace(/\.[a-zA-Z]+$/i, ".webp");
