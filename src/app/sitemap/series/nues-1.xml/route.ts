...
const SERIE_NAME = "nues"; // Cambiar para cada serie
...
async function getImages(): Promise<string[]> {
  ...
  const works: Work[] = seriesData.nues || [];
  return works.map(w => {
    // convert e.g. /serigraphie/nues/pierre-arnould-artiste-tondo-1.jpg
    // into /serigraphie/nues-mini/pierre-arnould-artiste-tondo-1.webp
    const webp = w.image.replace("/plexiglas/nues/", "/plexiglas/nues-mini/").replace(/\.[a-zA-Z]+$/i, ".webp");
